...
const apiMiddleware = ({ dispatch, getState }) => next => action => {
    ...
    if (urls && urls.length > 1) {
        ...
        APIResut = axios.request({
            url: getApiUrl(urls[0]), method, headers, [dataOrParams]: data, 'paramsSerializer': function (params) {
    ...
    APIResut.then(response => {
        ...
        if (error.response && showNotification) {
            dispatch(Notifications.error({ title: "Error", message: error.response.data.error, position: 'br', autoDismiss: 5 }));
        }
